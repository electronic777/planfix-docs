...
dateTime | [ DateTime](ПланФикс_API_Тип_данных_DateTime.md) | дата/время создания - не обязательный, по умолчанию - не изменяется | может заполняться, только если авторизация была сделана под сотрудником с правами администратора   
...
itemData.fieldId | int | идентификатор поля аналитики | идентификаторы можно получить методом [feild.id](ПланФикс_API_analitic.getOptions.md)  

itemData.value | string | значение поля аналитики |   

deletedAnalitics |  | список удаленных аналитик |   

deletedAnalitics.key | int | ключ данных удаляемой аналитики | данное поле становится доступным после выполнения [функции analitic.getData](ПланФикс_API_analitic.getData.md)  
