...
taskNewStatus | enum | этим комментарием меняется статус задачи на указанный | не обязательный параметр, перечень допустимых значений смотри в разделе [статусы задач](ПланФикс_API_Статусы_задач.md "ПланФикс API:Статусы задач")  
...
owner.id | int | идентификатор автора комментария | если это контакт - нужно использовать userid из ответа [contact.get](ПланФикс_API_contact.get.md "ПланФикс API contact.get")  

dateTime | [ DateTime](ПланФикс_API_Тип_данных_DateTime.md "ПланФикс API:Тип данных DateTime") | дата/время создания — не обязательный, по умолчанию текущие | может заполняться, только если авторизация была сделана под сотрудником с правами администратора   
...
itemData.fieldId | int | идентификатор параметра | идентификатор параметра равен [field.id](ПланФикс_API_analitic.getOptions.md "ПланФикс API analitic.getOptions")  
