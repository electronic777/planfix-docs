Сложные фильтры в [REST API](Интеграции/REST_API.md) ПланФикса применяются в методе «/user/list» при получении списка сотрудников. Фильтры сотрудников задаются следующим набором параметров: 
