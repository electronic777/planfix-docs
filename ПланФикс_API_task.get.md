...
status | enum | статус задачи | перечень допустимых значений смотри в разделе [статусы задач](ПланФикс_API_Статусы_задач.md "ПланФикс API:Статусы задач")  
...
dayType | enum | определяет тип дня | используется при type=EXACT. Допустимые значения смотри в разделе [тип дня для повторяющейся задачи](ПланФикс_API_Тип_дня_для_повторяющейся_задачи.md "ПланФикс API:Тип дня для повторяющейся задачи")  
