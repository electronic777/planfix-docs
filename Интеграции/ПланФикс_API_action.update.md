...
taskNewStatus | enum | этим комментарием меняется статус задачи на указанный | не обязательный параметр, перечень допустимых значений смотри в разделе [статусы задач](Задачи/ПланФикс_API_Статусы_задач.md), попытка поменять на неправильный статус или поменять статус не последним комментарием приведет к ошибке   
...
dateTime | [ DateTime](Интеграции/ПланФикс_API_Тип_данных_DateTime.md) | дата/время создания - не обязательный, по умолчанию - не изменяется | может заполняться, только если авторизация была сделана под сотрудником с правами администратора   

analitics |  | список обновляемых аналитик | не обязательный параметр   

analitics.analitic |  | узел, содержащий данные по аналитике |   

analitic.id | int | идентификатор аналитики | список доступных аналитик можно получить при помощи функции [analitic.getList](Интеграции/ПланФикс_API_analitic.getList.md)  
...
itemData.fieldId | int | идентификатор поля аналитики | идентификаторы можно получить методом [feild.id](Интеграции/ПланФикс_API_analitic.getOptions.md)  

itemData.value | string | значение поля аналитики |   

deletedAnalitics |  | список удаленных аналитик |   

deletedAnalitics.key | int | ключ данных удаляемой аналитики | данное поле становится доступным после выполнения [функции analitic.getData](Интеграции/ПланФикс_API_analitic.getData.md)  
