Функция для получения списка статусов процесса. Возвращает все статусы, которые присутствуют в наборе статусов процесса. Для получения статусов в который можно перевести задачу в данный момент существует функция [task.getPossibleStatusToChange](ПланФикс_API_task.getPossibleStatusToChange.md)
