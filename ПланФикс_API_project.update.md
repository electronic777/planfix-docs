...
status | enum | новый статус проекта | перечень допустимых значений для данного поля смотри в разделе константы [статусы проектов](ПланФикс_API_Статусы_проектов.md "ПланФикс API:Статусы проектов"). не обязательный параметр   
